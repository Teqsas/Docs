...
!!! info "Hinweis"
    Das INCLINOMETER ist ab Werk für den Betrieb mit der Bodenplatte nach unten kalibriert.

    Ist der Betrieb des INCLINOMETER mit einer anderen Ausrichtung der Bodenplatte gewünscht (oben, links oder rechts), ist hierzu eine Kalibrierung in der gewünschten Position notwendig.

    Sind alle Positionen einmal vollständig kalibriert worden, lässt sich das INCLINOMETER ohne weitere Kalibrierungen in allen Positionen nutzen.

Eine neue Kalibrierung kann notwendig werden, wenn das INCLINOMETER extremen Stößen ausgesetzt wurde (z. B. durch Herabfallen). Eine Kalibrierung ist notwendig, wenn das INCLINOMETER nicht mit der Bodenplatte nach unten betrieben werden soll, also Bodenplatte nach oben, Bodenplatte nach links oder rechts. Der Kalibriervorgang ist in allen Positionen bis auf die Ausrichtung der Bodenplatte identisch und wird daher in dieser Anleitung exemplarisch für die Variante Bodenplatte nach unten beschrieben.

Zusätzlich zu dem zu kalibrierenden INCLINOMETER wird folgendes Material benötigt:
...
<!-- TODO: Abbildung "Schritt 2 – INCLINOMETER auf Bezugsfläche, Laseraustritt nach links" einfügen -->

* Prüfen, ob die Bezugsfläche, auf der die Kalibrierung durchgeführt werden soll, in Waage ist.
* INCLINOMETER mit der Bodenplatte auf der Bezugsfläche positionieren. Der Laseraustritt zeigt nach links.
...
<!-- TODO: Abbildung "Schritt 4 – INCLINOMETER drehen, Laseraustritt nach rechts" einfügen -->

* INCLINOMETER so drehen, dass der Laseraustritt nach rechts zeigt.
...
<!-- TODO: Abbildung "Schritt 5 – INCLINOMETER mit Geodreieck auf +45° ausrichten" einfügen -->

* INCLINOMETER mit Hilfe des Geodreiecks so ausrichten, dass der Laser in einem 45°-Winkel nach oben scheint.
...
<!-- TODO: Abbildung "Schritt 6 – INCLINOMETER mit Geodreieck auf -45° ausrichten" einfügen -->

* INCLINOMETER mit Hilfe des Geodreiecks so ausrichten, dass der Laser in einem 45°-Winkel nach unten scheint.
...
### Schritt 7
Wird das INCLINOMETER ausschließlich mit der Bodenplatte nach unten betrieben, muss nichts weiter gemacht werden.
...
### Schritt 8
Wird der Betrieb des INCLINOMETER gewünscht, bei der die Bodenplatte eine andere Position einnimmt, muss die Kalibrierung fortgesetzt werden.

Die Reihenfolge der vier möglichen Positionen ist wie folgt festgelegt (Sichtweise auf den XLR-Stecker vom INCLINOMETER):
...
Jetzt kann das INCLINOMETER mit der Bodenplatte nach unten und der Bodenplatte nach oben betrieben werden. Genügen diese Positionen, kann der Kalibriermodus durch Abschalten mit der „Power-Taste" verlassen werden.

Werden auch die seitlichen Positionen gewünscht, müssen Schritt 2 bis Schritt 6 zunächst mit der Bodenplatte nach links (Sichtweise auf den XLR-Stecker am INCLINOMETER) wiederholt werden. Für die Ausrichtung mit Bodenplatte nach rechts (Sichtweise auf den XLR-Stecker am INCLINOMETER) muss die Kalibrierung Schritt 2 bis Schritt 6 ein weiteres Mal durchlaufen werden.

Nun kann das INCLINOMETER in allen vier Positionen betrieben werden.
